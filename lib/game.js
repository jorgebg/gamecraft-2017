...
class Game {
  ...
  randomPosition(radius) {
    let space = Math.floor(radius) + 10;
    return [
        d3.randomUniform(space, this.width-space)(),
        d3.randomUniform(space, this.height-space)(),
    ];
  }
  ...
  addField() {
      ...
      let radius = d3.randomUniform(50, 250)();
  ...
  startFieldSpawn() {
      ...
      // console.log(nFields < nPlayers * 1.5, nFields, '/', nPlayers * 1.5)
      if (nFields < nPlayers * 1.5) {
        let n = d3.randomUniform(1, 1 + nPlayers / 2)();
        for (let i=0; i < n; i++) {
          this.addField();
        }
      }
          ...
          for (let fieldId in this.fields) {
            ...
            if(body.overlaps(field.body)) {
              ...
              let distance = Math.sqrt(
                Math.pow(body.position[0] - field.body.position[0], 2) +
                Math.pow(body.position[1] - field.body.position[1], 2)
              )
              // let score = Math.floor(((field.shape.radius - distance) / field.shape.radius) * 2)
              // console.log(distance, field.shape.radius, field.shape.radius - distance);
              let score = 1;

              player.score += score;
              field.shape.radius -= score;
