...
class Game {
  ...
  randomPosition(radius) {
    return [
        radius + (Math.random()*this.width-radius),
        radius + (Math.random()*this.height-radius)
    ];
  }
  ...
  removeField(field) {
    console.log('Field destroyed', field.id, field.position);
    this.world.removeBody(field);
  ...
  getState() {
    ...
    for (let id in this.fields) {
      ...
      fields[id] = {
        x: field.position[0],
        y: this.height-field.position[1],
        radius: field.radius,
      }
    }
    return {
    ...
    setInterval(() => {
        ...
        for (let id in this.players) {
          ...
          for (let fieldId in this.fields) {
            ...
            if(body.overlaps(field)) {
              console.log(player.id, ' overlaps ', field.id);
              player.score += 10;
              field.radius -= 10;
              if (field.radius <= 0) {
